...
public class LinkedListContainer extends ReferenceCountingBase {
  private static void testOperations(
      java.util.function.Consumer<java.util.LinkedList<BasicType>> fn) {
  ...
  private static void testDoubleStream() {
    ...
    testOperations(values -> {
      assert values.size() == values.stream().mapToDouble(foobar1 -> {
        return foobar1.getValue();
      }).flatMap(i -> java.util.stream.DoubleStream.of(i, i, i)).skip(values.size())
    ...
    testOperations(values -> {
      assert 0 < java.util.stream.DoubleStream.generate(() -> (int) (Math.random() * 5)).limit(10)
          .sum();
    });
  }
  ...
  private static void testIntStream() {
    ...
    testOperations(values -> {
      assert values.size() == values.stream().mapToInt(foobar1 -> {
        return foobar1.getValue();
      }).flatMap(i -> java.util.stream.IntStream.of(i, i, i)).skip(values.size()).limit(values.size())
          .count();
    });
    testOperations(values -> {
      assert values.size() == values.stream().mapToInt(foobar1 -> {
  ...
  private static void testIteratorOperations() {
    ...
    testOperations(values -> {
      final java.util.Spliterator<com.simiacryptus.demo.refcount.BasicType> spliterator647 = values
          .spliterator();
  ...
  private static void testLongStream() {
    ...
    testOperations(values -> {
      assert values.size() == values.stream().mapToLong(foobar1 -> {
        return foobar1.getValue();
      }).flatMap(i -> java.util.stream.LongStream.of(i, i, i)).skip(values.size()).limit(values.size())
          .count();
    });
    testOperations(values -> {
      assert values.size() == values.stream().mapToLong(foobar1 -> {
  ...
  private static void testObjStream() {
    ...
    testOperations(values -> {
      assert values.size() == values
          .stream().collect(java.util.stream.Collectors.toMap(x -> {
            return x;
          }, x -> {
            return x.getValue();
          })).size();
    ...
    testOperations(values -> {
      assert values.size() == values
          .stream().collect(java.util.stream.Collectors.groupingBy(x -> {
            return x;
          })).size();
    });
    testOperations(values -> {
    ...
    testOperations(values -> {
      @NotNull
      java.util.ListIterator<com.simiacryptus.demo.refcount.BasicType> iter1056 = values
          .listIterator();
