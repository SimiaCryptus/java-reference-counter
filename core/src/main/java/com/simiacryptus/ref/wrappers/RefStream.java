...
public class RefStream<T> implements Stream<T> {
  ...
  public RefStream<T> filter(@NotNull Predicate<? super T> predicate) {
    inner = inner.filter((T t) -> predicate.test(RefUtil.addRef(t)));
    track(predicate);
    return this;
  }
  ...
  public Optional<T> max(@NotNull Comparator<? super T> comparator) {
    track(comparator);
    return inner.max((T o1, T o2) -> comparator.compare(RefUtil.addRef(o1), RefUtil.addRef(o2))).map(this::getRef);
  }

  @Override
  public Optional<T> min(@NotNull Comparator<? super T> comparator) {
    track(comparator);
    return inner.min((T o1, T o2) -> comparator.compare(RefUtil.addRef(o1), RefUtil.addRef(o2))).map(this::getRef);
  }
  ...
  public RefStream<T> sorted() {
    inner = inner.sorted((a, b) -> ((Comparable<T>) a).compareTo(RefUtil.addRef(b)));
    return this;
  }
  ...
  public RefStream<T> sorted(@NotNull Comparator<? super T> comparator) {
    inner = inner.sorted((T o1, T o2) -> comparator.compare(RefUtil.addRef(o1), RefUtil.addRef(o2)));
    return this;
  }
